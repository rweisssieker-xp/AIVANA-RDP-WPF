...
use crate::standard_security::StandardSecurityContext;
use crate::{
...
pub enum ClientConnectorState {
    ...
    ChannelConnection {
        io_channel_id: u16,
        selected_protocol: nego::SecurityProtocol,
        server_security: gcc::ServerSecurityData,
        channel_connection: ChannelConnectionSequence,
    },
    StandardSecurityExchange {
        io_channel_id: u16,
        user_channel_id: u16,
        server_security: gcc::ServerSecurityData,
    },
    SecureSettingsExchange {
        io_channel_id: u16,
        user_channel_id: u16,
        standard_security: Option<StandardSecurityContext>,
    },
...
impl State for ClientConnectorState {
    fn name(&self) -> &'static str {
        match self {
            ...
            Self::StandardSecurityExchange { .. } => "StandardSecurityExchange",
...
impl Sequence for ClientConnector {
    fn next_pdu_hint(&self) -> Option<&dyn PduHint> {
        match &self.state {
            ...
            ClientConnectorState::StandardSecurityExchange { .. } => None,
    ...
    fn step(&mut self, input: &[u8], output: &mut WriteBuf) -> ConnectorResult<Written> {
        let (written, next_state) = match mem::take(&mut self.state) {
            ...
            ClientConnectorState::BasicSettingsExchangeWaitResponse {
                ...
                if !selected_protocol.is_standard_rdp_security()
                    && client_gcc_blocks.security == gcc::ClientSecurityData::no_security()
            ...
            ClientConnectorState::ChannelConnection {
                ...
                let next_state = if let ChannelConnectionState::AllJoined { user_channel_id } = channel_connection.state
                {
                    ...
                    if selected_protocol.is_standard_rdp_security() && server_security.server_random.is_some() {
                        ClientConnectorState::StandardSecurityExchange {
                            io_channel_id,
                            user_channel_id,
                            server_security,
                        }
            ...
            ClientConnectorState::StandardSecurityExchange {
                ...
                debug!("Standard RDP Security Exchange");

                let (standard_security, security_exchange) =
                    StandardSecurityContext::new(&server_security)?;
                let written =
                ...
                (
                    ...
                    ClientConnectorState::SecureSettingsExchange {
                        io_channel_id,
                        user_channel_id,
                        standard_security: Some(standard_security),
                    },
                )
            }
            ...
            ClientConnectorState::SecureSettingsExchange {
                ...
                let written = if let Some(standard_security) = &mut standard_security {
                    let client_info = encode_vec(&client_info).map_err(ConnectorError::encode)?;
                    let encrypted = standard_security.encrypt_user_data(&client_info)?;
                    crate::standard_security::encode_send_data_request_raw(
                        user_channel_id,
                        io_channel_id,
                        encrypted,
                        output,
            ...
            ClientConnectorState::LicensingExchange {
                ...
                let written = if let Some(standard_security) = &mut standard_security {
                    license_exchange.step_with_standard_security(input, output, standard_security)?
                } else {
                    license_exchange.step(input, output)?
                };
            ...
            ClientConnectorState::CapabilitiesExchange {
                ...
                let input = if input.is_empty() {
                    Vec::new()
                } else if let Some(standard_security) = &mut standard_security {
                    standard_security.decrypt_server_frame(input)?
                } else {
                    input.to_vec()
                };
                let mut local_output = WriteBuf::new();
                let written = connection_activation.step(&input, &mut local_output)?;
                write_standard_output(output, local_output, written, standard_security.as_mut())?;
...
fn write_standard_output(
    output: &mut WriteBuf,
    local_output: WriteBuf,
    written: Written,
    ...
    let bytes = local_output.filled();
    let bytes = if let Some(standard_security) = standard_security {
        standard_security.encrypt_client_frame(bytes)?
    } else {
        bytes.to_vec()
    };
    ...
    Ok(ClientGccBlocks {
        ...
        security: ClientSecurityData {
            encryption_methods: if selected_protocol.is_standard_rdp_security() {
                EncryptionMethod::BIT_128 | EncryptionMethod::BIT_56 | EncryptionMethod::BIT_40
            } else {
